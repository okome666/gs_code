...
if($status==false) {
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){ 
    $temp .= "<p>";
    $temp .= $result["indate"].'　　'.'<a href="detail.php?id='.$result["id"].'">'.'編集'.'</a>'.' '.'<a href="delete.php?id='.$result["id"].'">'.'削除'.'</a>'.'<br>'.$result["name"].'　'.$result["vacant"].'　'.$result["place"].'<br>'.'<a href='.$result["URL"].'>ポートフォリオ</a>';
    $temp .= "</p>";
